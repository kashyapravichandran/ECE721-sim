...
void pipeline_t::rename2() {
   ...
   if (!RENAME2[0].valid ||	// First stall condition: There isn't a current rename bundle.
	...
	unsigned int count_branches=0, count_destination=0;
   ...
   for (i = 0; i < dispatch_width; i++) {
	  ...
	  {
	  	count_destination++;	
	  }  // check checkpoint and C_valid increase counter
	  if (PAY.buf[index].checkpoint)
	  {
   	...
   	// call the renamer class in your code to check the number of branches and the number of elements in the free list.
   ...
   // This is achieved by doing nothing and proceeding to the next statements.
	if(REN->stall_reg(count_destination))
		return ; // if there are no enough resources. 
	if(REN->stall_branch(count_branches))
		return;
   //
