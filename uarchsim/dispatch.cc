...
void pipeline_t::dispatch() {
   ...
   for (i = 0; i < dispatch_width; i++) {
	  ...
	  bool destination;
	  unsigned int pc, log_reg, phys_reg;
	  
	  destination=PAY.buf[index].C_valid;
	  ...
	  PAY.buf[index].AL_index=REN->dispatch_inst(destination, log_reg, phys_reg, load_flag, store_flag, branch_flag, amo_flag, csr_flag, pc);
      ...
      switch (PAY.buf[index].iq) {
         case SEL_IQ:{
			...
			//unsigned int A_valid, B_valid, D_valid, A_tag, B_tag, D_tag, A_ready, B_ready, D_ready;
			//unsigned int branch_mask, lane_id;
			//A_valid=PAY.buf[index].A_valid;
			//B_valid=PAY.buf[index].B_valid;
			//D_valid=PAY.buf[index].D_valid;
			//A_tag=PAY.buf[index].A_phys_reg;
			//B_tag=PAY.buf[index].B_phys_reg;
			//D_tag=PAY.buf[index].D_phys_reg;
			//branch_mask = 
			//lane_id=;
